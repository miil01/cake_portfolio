...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (feedbackBtn && feedbackTextarea) {
    feedbackBtn.addEventListener("click", (e) => {
      ...
      emailjs.send('service_01tm3no', 'template_g2ttiuk', {
        ...
        alert("送信失敗 ❌: " + JSON.stringify(err));
      });
    });
  }

  // ここから既存の slicesクリック処理、モーダル、BGM、時計なども全部ここに入れる
});  // =============================
  // DOM要素取得
  // =============================
  const slices = document.querySelectorAll(".slice");
  ...
  if (feedbackBtn && feedbackTextarea) {
    feedbackBtn.addEventListener("click", (e) => {
      ...
      emailjs.send('service_tvsve7q', 'template_abcd1234', {
        user_name: 'mina', // 任意
        feedback: feedbackText
      }).then((response) => {
  ...
  // ====== ここから既存のケーキホイールやモーダル処理 ======
  // 省略：既存の slices, modal, clock, BGM, endIcon などの処理
